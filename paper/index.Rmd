...
```{r editing, fig.cap="Examples of image manipulations: (A) original image, (B) mirrored, (C) cropped to 75%, (D) resized to 75%, (E) rotated 180 degrees, (F) 30 pixels of black padding added, and (G) greyscale,."}
...
You can automatically delineate faces with a simpler template (Fig.\ \@ref(fig:auto-delin)) using services provided through [Face++](https://www.faceplusplus.com/landmarks/). 
...
```{r auto-delin, fig.cap="Delineation templates: (A) manual delineations using the FRL template and (B) automatic delineations using the Face++ template."}
plot_rows(A = draw_tem(f), B = draw_tem(fpp_tem))
```

A study comparing the accuracy of four common measures of face shape (sexual dimorphism, distinctiveness, bilateral asymmetry, and facial width to height ratio) between automatic and manual delineation concluded that automatic delineation had higher replicability and good correlations with manual delineation [@jones2020facial]. However, around 2% of images had noticeably inaccurate automatic delineation, which should be screened for by outlier detection and visual inspection. 
...
```{r avg-comp, fig.cap="Averages of 5 images made using (A) the full 189-point manual template and (B) the reduced 106-point automatic template."}
frl_avg <- avg(f)
fpp_avg <- avg(fpp_tem)
...
```{r avg-texture, fig.cap="An average of 5 faces created (A) with texture averaging and (B) without."}
...
```{r trans-vs-morph, fig.cap="Morphing versus transforming: (A, B) composite images, (C, D) averages of the composites with an individual image, (E, F) transforms of an individual image along the male-female continuum."}
