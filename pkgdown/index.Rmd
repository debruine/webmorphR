...
In this example, we'll load a few faces from the CC-BY licensed [Face Research Lab London Set](https://doi.org/10.6084/m9.figshare.5047666.v3), average them together, create a figure


Load 6 faces. If you haven't already downloaded the demo stimulus sets, this code will prompt you to download them. The demo sets are large, so this takes a minute or so.
...
First, transform images to make them more average. The `trans()` function also sends images to webmorph.org, so can take a minute and requires and internet connection.
...
Next, mask and crop images. Use `names()` to check the order of the images in the `dist_avg` object 
...
```{r}
rainbow <- c("#983E82", "#E2A458", "#F5DC70", "#59935B", "#467AAC", "#61589C") |> rep(times = 2)

stimuli <- dist_avg[c(7:12, 1:6)] |>
...
Easily create figures to illustrate your research. The code below adds 120 pixels of black padding to the top of each image, labels them with their name, and plots all images in two rows.
