...
Set `label` to a vector of labels to use. The default font size used by ggplot is pretty small, so increase `size`. I can't help you with how to set that argument; I just use trial-and-error. Labels can be positioned with the `x` and `y` arguments. Unlike ggplot, x and y values less than or equal to 1 are interpreted as percentages.
