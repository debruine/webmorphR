...
There are two labelling functions. The original one, `label()` uses syntax like `magick::image_annotate()` to configure labels. The new one, `gglabel()`, uses syntax like `ggplot2::annotate()` to configure labels.
...
Alternatively, set `label` to a vector of labels to use. Labels can be positioned with the `x` and `y` arguments. Unlike ggplot, x and y values less than or equal to 1 are interpreted as percentages.
Adjust label alignment with `hjust` and `vjust`. I can't help you with the ggplot `size` argument; I just use trial-and-error.
