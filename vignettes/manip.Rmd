...
```{r}
image_centered <- offset  |> 
  rotate(degrees, rotate_around = "image", fill = "pink")

image_centered |> plot(nrow = 1)
```

Set `rotate_around = "tem"` to rotate around the centre of the template.

```{r}
tem_centered <- offset |> 
  rotate(degrees, rotate_around = "tem", fill = "pink")

tem_centered |> plot(nrow = 1)
```
...
image_centered <- img |> 
  ...
  rotate(degrees, rotate_around = "image", fill = "dodgerblue")|>
  pad(30, 0, 0, 0) |> 
  label('rotate_around = "image"', size = 20)

# rotate around centre of template
tem_centered <- img |> 
  mask(fill = "hotpink") |>
  rotate(degrees, rotate_around = "tem", fill = "hotpink")|>
  pad(30, 0, 0, 0) |> 
  label('rotate_around = "tem"', size = 20)

# rotate around left eye point
lefteye_centered <- img |>
  mask(fill = "orange") |>
  rotate(degrees, rotate_around = 0, fill = "orange")|>
  pad(30, 0, 0, 0) |> 
  label('rotate_around = 0 (left eye)', size = 20)

# rotate around right eye point
righteye_centered <- img |> 
  mask(fill = "purple") |>
  rotate(degrees, rotate_around = 1, fill = "purple")|>
  pad(30, 0, 0, 0) |> 
  label('rotate_around = 1 (right eye)', size = 20)
...
If you set `keep_size = FALSE`, the image and template will be rotated and keep the maximum rotated size (so it doesn't matter what you set `rotate_around` to).

```{r}
no_keep <- offset |> 
  rotate(degrees, keep_size = FALSE, fill = "pink")
...
fit_crop <- to_size(multisize, 400, 400, crop = TRUE)
plot(fit_crop)
```
...
```{r fig-rel-crop}
rel_crop <- to_size(multisize, 400, 400, crop = TRUE, keep_rels = TRUE)
plot(rel_crop)
```
