...
opengraph:
  image: 
    src: "https://debruine.github.io/webmorphR/articles/images/manip.png"
  twitter:
    card: summary
...
plot(fit_pad)
```
Set `keep_rels = TRUE` to brings the largest image to fit in the new size, resize the others proportionally, and pad.

```{r fig-rel-pad}
rel_pad <- to_size(multisize, 300, 400, fill = "dodgerblue", keep_rels = TRUE)
plot(rel_pad)
```

Set `crop = TRUE` to bring all images to cover new size and crop the "overhang". There is no need to set the fill color for padding when crop is TRUE.
