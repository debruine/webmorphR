...
maskpos <- mask(stimuli[1],  expand = 20)
maskneg <- mask(stimuli[1],  expand = -20)
revmask0 <- mask(stimuli[1], expand = 0, reverse = TRUE)
revmaskpos <- mask(stimuli[1], expand = 20, reverse = TRUE)
revmaskneg <- mask(stimuli[1],expand = -20, reverse = TRUE)

# plot
c(mask0, maskpos, maskneg, revmask0, revmaskpos, revmaskneg) |> 
  crop_tem() |> draw_tem() |>
  pad(80, 0, 0, 0) |>
  label(c("expand = 0", "expand = 20", "expand = -20")) |>
  plot(nrow = 2)
```
